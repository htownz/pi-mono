...
def fair_probability(
    ...
    Conclusive states map to deterministic ranges with a tiny epsilon for
    settlement-source risk. Forecast-dependent states use the hourly forecast's
    own envelope plus a residual buffer to bound where the remaining extreme
    can fall.
    """
    eps = 0.02
    ...
    if state is ContractState.BRACKET_HIT_VULNERABLE:
        ...
        if contract.metric is Metric.HIGH:
            ...
            return max(0.0, p - 0.08), min(1.0, p + 0.08)
        else:
            assert bracket.lo is not None
    ...
    if contract.metric is Metric.HIGH:
        ...
        return _bracket_overlap_prob(bracket, proj_lo, proj_hi)
